...
export function DashboardExperience() {
  ...
  return <>
    ...
    <section id="my-area" className="mt-10" aria-labelledby="my-area-heading">
      {saved === undefined ? <AreaSkeleton /> : choosing || !saved ? <LocationSelector areas={areas.data} subAreas={subAreas.data} areaId={areaId} subAreaId={subAreaId} areasLoading={areas.loading} areasError={areas.error} subAreasLoading={subAreas.loading} subAreasError={subAreas.error} canCancel={Boolean(saved)} onAreaChange={(area) => { setAreaId(area.id); setSubAreaId(null); }} onSubAreaChange={(subArea) => setSubAreaId(subArea.id)} onRetryAreas={areas.retry} onRetrySubAreas={subAreas.retry} onConfirm={confirmLocation} onCancel={() => setChoosing(false)} /> : <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[0.16em] text-teal-700">Your Area</p><h2 id="my-area-heading" className="mt-1 text-2xl font-black tracking-tight text-slate-950">{localityMatchesSaved ? `${locality.sub_area.name}, ${locality.sub_area.area.name}` : `${saved.subAreaName}, ${saved.areaName}`}</h2><p className="mt-2 text-sm text-slate-600">Community-reported current picture — not utility-provider confirmation.</p><div className="mt-2"><LiveIndicator paused={liveState.paused} refreshing={liveState.refreshing} failed={liveState.failed} onRefresh={() => setRefreshKey((value) => value + 1)} /></div></div><button type="button" onClick={() => { setAreaId(saved.areaId); setSubAreaId(saved.subAreaId); setChoosing(true); }} className="min-h-11 rounded-xl border border-slate-200 px-4 text-sm font-bold text-slate-700 hover:bg-slate-50">Change</button></div>
        ...
        <div className="mt-5 flex flex-wrap gap-3"><Link href="/report" className="inline-flex min-h-12 items-center rounded-xl bg-slate-950 px-5 font-bold text-white hover:bg-teal-800">Help your area — report now</Link>{locality?.sub_area.area.slug && locality.sub_area.slug ? <LocalityLink areaSlug={locality.sub_area.area.slug} subAreaSlug={locality.sub_area.slug}>View full locality history</LocalityLink> : null}</div>
      </section>}
    </section>
    <section className="mt-10" aria-labelledby="struggling-heading"><div className="flex items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[0.16em] text-rose-700">Live locality signals</p><h2 id="struggling-heading" className="mt-1 text-2xl font-black tracking-tight text-slate-950">Currently Struggling</h2></div><Link href="/areas" className="text-sm font-bold text-teal-800">Browse areas →</Link></div>{dashboardError ? <InlineError message="Couldn’t load live locality updates." onRetry={() => setRefreshKey((value) => value + 1)} /> : dashboard ? <ProjectionList items={dashboard.struggling} empty={strugglingEmptyStateCopy} /> : <ListSkeleton />}</section>
    ...
    {dashboard ? <section className="mt-10" aria-labelledby="changes-heading"><h2 id="changes-heading" className="text-2xl font-black tracking-tight text-slate-950">Recent Changes</h2><p className="mt-1 text-sm text-slate-600">Recent projection updates, never individual reporter activity.</p><div className="mt-4"><ProjectionList items={dashboard.recent_changes} empty="No recent locality projections yet." /></div></section> : null}
  </>;
}
